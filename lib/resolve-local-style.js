...
export function resolveIncludes (style, theme) {
  if (style.hasOwnProperty ('includes')) {
    let merge = {};
    Object.keys (style).forEach (name => {
      if (name === 'includes') {
        merge = appendIncludes (merge, style.includes.map (include => theme.styles[include]));
      } else {
...
export function mergeStyleProperties (left, right) {
...
export function resolveLocalStyle (styleMap, localStyle, theme, current = {}) {
