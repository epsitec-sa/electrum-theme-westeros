...
const {resolveLocalStyle} = require('./resolve-local-style.js');
...
class Styles {
  constructor(key, def) {
    if (key !== secretKey) {
      throw new Error(
        'Do not call Styles constructor directly; use Styles.create instead'
      );
    }
    if (typeof def !== 'function' || def.length > 2) {
      throw new Error(
        'Styles must be defined by a function taking 0, 1 or 2 arguments: (theme, props) => {}'
      );
    }
    this._def = def;
